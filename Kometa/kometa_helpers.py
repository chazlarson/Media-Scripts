...
def get_size(the_lib, tgt_class=None, tgt_filter=None):
    ...
    if tgt_filter is not None:
        temp_var = the_lib.search(libtype=item_class, filters=tgt_filter)
        lib_size = len(temp_var)
    else:
        temp_var = the_lib.search(libtype=item_class)
        lib_size = len(temp_var)
...
def get_all_from_library(the_lib, tgt_class=None, tgt_filter=None):
    ...
    while lib_size is None or c_start <= lib_size:
        if tgt_filter is not None:
            results.extend(the_lib.search(libtype=item_class, maxresults=c_size, container_start=c_start, container_size=lib_size, filters=tgt_filter))
        else:
...
def load_and_upgrade_env(file_path):
    ...
    if os.getenv("PLEXAPI_AUTH_SERVER_BASEURL") is None or os.getenv("PLEXAPI_AUTH_SERVER_BASEURL") == 'https://plex.domain.tld':
        print("You must specify PLEXAPI_AUTH_SERVER_BASEURL in the .env file.", 'info', 'a')
        status = -1

    if os.getenv("PLEXAPI_AUTH_SERVER_TOKEN") is None or os.getenv("PLEXAPI_AUTH_SERVER_TOKEN") == 'PLEX-TOKEN':
        print("You must specify PLEXAPI_AUTH_SERVER_TOKEN in the .env file.", 'info', 'a')
        status = -1
