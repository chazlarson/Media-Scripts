...
is_docker = get_env("MS_DOCKER", False, arg_bool=True)
...
while i < len(unknown):
    ...
    if test_var.startswith("--kometa-") or test_var in ["-pu", "--plex-url", "-pt", "--plex-token"]:
...
logger = MyLogger("Media Scripts", default_dir, run_args["width"], run_args["debug"], run_args["trace"], run_args["log-requests"])
...
def start(attrs):
    ...
    logger.separator()
    logger.info("")
    logger.info_center("  __  __          _ _        _____           _       _       ")
    logger.info_center(" |  \\/  |        | (_)      / ____|         (_)     | |      ")
    logger.info_center(" | \\  / | ___  __| |_  __ _| (___   ___ _ __ _ _ __ | |_ ___ ")
    logger.info_center(" | |\\/| |/ _ \\/ _` | |/ _` |\\___ \\ / __| '__| | '_ \\| __/ __|")
    logger.info_center(" | |  | |  __/ (_| | | (_| |____) | (__| |  | | |_) | |_\\__ \\")
    logger.info_center(" |_|  |_|\\___|\\__,_|_|\\__,_|_____/ \\___|_|  |_| .__/ \\__|___/")
    logger.info_center("                                              | |            ")
    logger.info_center("                                              |_|            ")
...
# }
    ...
    for akey, adata in arguments.items():
        if isinstance(adata["help"], str):
            ...
            logger.debug(f"--{akey} (KOMETA_{akey.upper()}): {ext}{run_args[akey]}{ext}")
    logger.debug("")
